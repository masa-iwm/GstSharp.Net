...
/// </summary>
internal static partial class GstWebRTCModule
{
    /// <summary>
    /// Runs before any code of this assembly: it hooks
    /// <see cref="NativeLoader"/> up, so that the <c>GstWebRTC</c> imports of
    ...
    {
        NativeLoader.EnsureRegistered(typeof(GstWebRTCModule).Assembly);
        TypeRegistry.RegisterModule(new NativeModule("GstWebRTC", CreateEntries()));
    }
}
