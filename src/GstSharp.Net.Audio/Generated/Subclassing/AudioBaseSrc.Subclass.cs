...
public unsafe partial class AudioBaseSrc
{
    ...
    public static new Gst.GObject.SubclassType DefineSubclass<TSelf>(
        ...
        Action<Gst.GObject.ClassConfig> configureClass,
        params Gst.GObject.VfuncOverride[] overrides)
        where TSelf : AudioBaseSrc, Gst.GObject.IManagedSubclass<TSelf> =>
    ...
    /// class that <c>GstAudioBaseSrc</c> does not derive from, an interface is declared
    ...
    /// </summary>
    /// <remarks>
    /// <para>The source parents the answer and takes a reference of its own (gst_object_set_parent
    /// sinks it, gstaudiobasesrc.c:1110-1111), so the wrapper keeps working and no reference is
    /// added on the way out. Answer a ring buffer that has no parent yet and keep no extra
    ...
    private static nint CreateRingbufferTrampoline(nint src)
    {
        try
        {
            if (Gst.GObject.Object.TryGetOrFabricate(src) is not AudioBaseSrc managed)
            {
