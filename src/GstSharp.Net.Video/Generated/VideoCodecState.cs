...
public sealed unsafe partial class VideoCodecState : Gst.GObject.Boxed
{
    ...
    /// <summary>Wraps a <c>GstVideoCodecState</c> that GStreamer keeps owning, for the length of one call.</summary>
    ...
    /// </remarks>
    internal static VideoCodecState Borrow(nint handle) => new(new Gst.Interop.Borrowed(handle));

    /// <summary>Wraps a <c>GstVideoCodecState</c> the caller keeps owning.</summary>
    /// <param name="borrowed">The value that is lent to managed code.</param>
    private VideoCodecState(Gst.Interop.Borrowed borrowed)
