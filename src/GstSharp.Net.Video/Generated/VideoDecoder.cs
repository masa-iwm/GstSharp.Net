...
public abstract unsafe partial class VideoDecoder : Gst.Element
{
    ...
    /// <summary>Answers a copy of the <c>input_segment</c> field of <c>GstVideoDecoder</c>.</summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// The library rewrites the field under the STREAM_LOCK of the decoder, a recursive mutex
    /// of its own, and OBJECT_LOCK for the flags of an instant rate change, which managed code
    /// cannot take, so the copy is only guaranteed consistent when it is read on the streaming
    /// thread, inside
    ...
    public Gst.Segment GetInputSegment()
    {
        Gst.Segment value = Gst.Segment.FromNative(
            Handle + Gst.Video.VideoDecoderOwnFieldsRaw.OwnOffset + Gst.Video.VideoDecoderOwnFieldsRaw.InputSegmentOffset,
            Gst.Interop.Transfer.None)
            ?? throw new System.InvalidOperationException("The 'input_segment' field of GstVideoDecoder is null.");
        System.GC.KeepAlive(this);
        return value;
    }

    /// <summary>Answers a copy of the <c>output_segment</c> field of <c>GstVideoDecoder</c>.</summary>
    /// <remarks>
    /// <para>
    /// The structure is embedded in the instance this wrapper points at. What comes
    /// back is a copy of it that the caller owns and disposes, so it stays good after
    ...
    {
        Gst.Segment value = Gst.Segment.FromNative(
            Handle + Gst.Video.VideoDecoderOwnFieldsRaw.OwnOffset + Gst.Video.VideoDecoderOwnFieldsRaw.OutputSegmentOffset,
            Gst.Interop.Transfer.None)
            ?? throw new System.InvalidOperationException("The 'output_segment' field of GstVideoDecoder is null.");
        System.GC.KeepAlive(this);
        return value;
    }
