...
public sealed unsafe partial class BufferList : Gst.MiniObject
{
    ...
    /// <summary>Wraps a <c>GstBufferList</c> that GStreamer keeps owning, for the length of one call.</summary>
    ...
    /// </remarks>
    internal static BufferList Borrow(nint handle) => new(new Gst.Interop.Borrowed(handle));

    /// <summary>Wraps a <c>GstBufferList</c> the caller keeps owning.</summary>
    /// <param name="borrowed">The mini object that is lent to managed code.</param>
    private BufferList(Gst.Interop.Borrowed borrowed)
        : base(borrowed)
    {
    }
