...
public sealed unsafe partial class Iterator : Gst.GObject.Boxed
{
    ...
    /// <summary>Wraps a <c>GstIterator</c> that GStreamer keeps owning, for the length of one call.</summary>
    ...
    /// </remarks>
    internal static Iterator Borrow(nint handle) => new(new Gst.Interop.Borrowed(handle));

    /// <summary>Wraps a <c>GstIterator</c> the caller keeps owning.</summary>
    /// <param name="borrowed">The value that is lent to managed code.</param>
    private Iterator(Gst.Interop.Borrowed borrowed)
