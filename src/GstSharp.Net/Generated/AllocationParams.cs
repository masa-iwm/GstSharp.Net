...
public sealed unsafe partial class AllocationParams : Gst.GObject.Boxed
{
    ...
    /// <summary>Wraps a <c>GstAllocationParams</c> that GStreamer keeps owning, for the length of one call.</summary>
    ...
    /// </remarks>
    internal static AllocationParams Borrow(nint handle) => new(new Gst.Interop.Borrowed(handle));

    /// <summary>Wraps a <c>GstAllocationParams</c> the caller keeps owning.</summary>
    /// <param name="borrowed">The value that is lent to managed code.</param>
    private AllocationParams(Gst.Interop.Borrowed borrowed)
