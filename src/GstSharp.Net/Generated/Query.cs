...
public sealed unsafe partial class Query : Gst.MiniObject
{
    ...
    /// <summary>Wraps a <c>GstQuery</c> that GStreamer keeps owning, for the length of one call.</summary>
    ...
    /// </remarks>
    internal static Query Borrow(nint handle) => new(new Gst.Interop.Borrowed(handle));

    /// <summary>Wraps a <c>GstQuery</c> the caller keeps owning.</summary>
    /// <param name="borrowed">The mini object that is lent to managed code.</param>
    private Query(Gst.Interop.Borrowed borrowed)
        : base(borrowed)
    {
    }
