...
public sealed unsafe partial class Promise : Gst.MiniObject
{
    ...
    /// <summary>Wraps a <c>GstPromise</c> that GStreamer keeps owning, for the length of one call.</summary>
    ...
    /// </remarks>
    internal static Promise Borrow(nint handle) => new(new Gst.Interop.Borrowed(handle));

    /// <summary>Wraps a <c>GstPromise</c> the caller keeps owning.</summary>
    /// <param name="borrowed">The mini object that is lent to managed code.</param>
    private Promise(Gst.Interop.Borrowed borrowed)
        : base(borrowed)
    {
    }
