...
public sealed unsafe partial class Event : Gst.MiniObject
{
    ...
    /// <summary>Wraps a <c>GstEvent</c> that GStreamer keeps owning, for the length of one call.</summary>
    ...
    /// </remarks>
    internal static Event Borrow(nint handle) => new(new Gst.Interop.Borrowed(handle));

    /// <summary>Wraps a <c>GstEvent</c> the caller keeps owning.</summary>
    /// <param name="borrowed">The mini object that is lent to managed code.</param>
    private Event(Gst.Interop.Borrowed borrowed)
        : base(borrowed)
    {
    }
