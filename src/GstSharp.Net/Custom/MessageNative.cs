...
/// <remarks>
/// <para>
/// <c>gst_message_parse_error</c> and <c>gst_message_parse_warning</c> hand a
/// <c>GError</c> out through a <c>GError**</c>. The generator does not emit
/// those signatures, because a <c>GError</c> that is a result rather than a
/// failure of the call has no place in the throwing convention of the
/// generated bindings: an error message is a value the application inspects,
/// not an exception the binding raises. Both entry points therefore belong on
/// the skip list of <c>girs/overlays/fixups.json</c> once the function emitter
/// covers them, so that there is a single way to read them.
/// </para>
/// <para>
