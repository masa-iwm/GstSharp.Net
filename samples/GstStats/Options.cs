...
/// <summary>
/// The command line of the sample: the one option of <c>gst-stats-1.0</c>, the
...
internal sealed class Options
{
    ...
    internal const string ProgramName = "GstStats";
    ...
    /// <summary>
    /// Gets the regular expression that reads a log line, if the command line
    /// carried one. Group 1 has to capture the text of the tracer structure.
    /// </summary>
    internal string? TracerRegexp { get; private set; }

    /// <summary>
    /// Gets a value indicating whether a log that yielded no tracer record ends
    /// the run with a non-zero exit code. This is not an option of
    /// <c>gst-stats-1.0</c>, which always exits 0 once it has a file to read; it
    /// exists so that an automated run can gate on the result.
    /// </summary>
    internal bool FailIfEmpty { get; private set; }

    /// <summary>Gets the log files that were named on the command line.</summary>
    ...
    internal static string Usage =>
        ...
        Application Options:
          -t, --tracer-regexp=REGEXP       Custom regular expression to extract tracer data.
                                           .NET syntax rather than Perl syntax, and group 1 has
                                           to capture the text of the structure
        ...
        Sample Options:
              ...
              --fail-if-empty              Exit with 1 when the log carried no tracer record at
                                           all. Not a gst-stats option; it exists so that an
    ...
    internal static Options Parse(string[] arguments)
    {
        ...
        for (int i = 0; i < arguments.Length; i++)
        {
            ...
            switch (name)
            {
                ...
                case "-t" or "--tracer-regexp":
                    options.TracerRegexp = ValueOf(arguments, ref i, name, inlineValue);
                    break;
