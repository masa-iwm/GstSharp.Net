...
/// <summary>
/// The command line of the sample: the options of <c>gst-discoverer-1.0</c>
/// that the synchronous path has, the <c>--gst-*</c> options that are handed to
...
internal sealed class Options
{
    ...
    internal const string ProgramName = "GstDiscoverer";

    /// <summary>The timeout per URI the C tool uses when none is given.</summary>
    ...
    /// <summary>Gets a value indicating whether the table of contents is printed.</summary>
    internal bool ShowToc { get; private set; }

    /// <summary>
    /// Gets a value indicating whether the verbose properties are printed: the
    /// caps of every stream, the additional info structure, and every tag.
    /// </summary>
    internal bool Verbose { get; private set; }

    /// <summary>
    /// Gets a value indicating whether the discoverer may answer from its cache
    /// rather than by running a pipeline.
    /// </summary>
    ...
    /// <summary>
    /// Gets a value indicating whether a URI that could not be discovered ends
    /// the run with a non-zero exit code. This is not an option of
    /// <c>gst-discoverer-1.0</c>, which exits 0 whatever it found; it exists so
    /// that an automated run can gate on the result.
    /// </summary>
    internal bool FailOnError { get; private set; }

    /// <summary>Gets the URIs and file names to discover.</summary>
    ...
    internal static string Usage =>
        $"""
        Usage:
          {ProgramName} [OPTION...] - discover files synchronously with GstDiscoverer
        ...
        Sample Options:
              ...
              --fail-on-error              Exit with 1 when a URI could not be discovered. Not a
                                           gst-discoverer option; it exists so that an automated
                                           run can gate on the result rather than only on the
                                           absence of a crash
        """;
    ...
    internal static Options Parse(string[] arguments)
    {
        ...
        for (int i = 0; i < arguments.Length; i++)
        {
            ...
            if (literal)
            {
                options._uris.Add(argument);
                continue;
            }

            // Everything after "--" is a URI, which is how one that starts with
            // a dash is passed.
            if (argument == "--")
            {
                literal = true;
            ...
            switch (name)
            {
                ...
                case "-t" or "--timeout":
                {
                    string text = ValueOf(arguments, ref i, name, inlineValue);

                    // G_OPTION_ARG_INT, which is what the C tool declares, and
                    // its parser reports exactly this sentence for a token that
                    // is not one.
                    options.TimeoutSeconds =
                        int.TryParse(text, NumberStyles.AllowLeadingSign, CultureInfo.InvariantCulture, out int seconds)
                            ? seconds
                            : throw new OptionException($"Cannot parse integer value \"{text}\" for {name}");
                    break;
                }
                ...
                default:
                    // A token that is not an option is a URI or a file name. One
                    // that looks like an option and is not known is the error the
