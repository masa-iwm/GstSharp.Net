...
of: `videotestsrc`, `compositor`, `videoconvertscale`, `videorate` and
`capsfilter` from the base plugins and `videoflip`, `videocrop` and
`deinterlace` from the good ones. It prints how many children the
...
`null` — a null answer, which the binding has to substitute an `identity` for,
