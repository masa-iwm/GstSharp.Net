...
8. No lock-taking accessor inside a member the library can reach while it
   ...
   wait for the lock the caller already holds. The one member that takes a
   library lock through its field address is the carve-out that
   `BaseSink.PrerollLock()` makes for a thread a sink owns, outside every
   override; see "Prerolling from a thread of your own" in
   [`docs/subclassing.md`](subclassing.md).
