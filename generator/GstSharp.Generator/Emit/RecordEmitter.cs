...
internal sealed class RecordEmitter
{
    ...
    internal GeneratedFile? Emit(ModuleInfo module, GirNamespace ns, GirRecord record)
    {
        ...
        WriteHeader(writer, module, ns, kind, layout, NeedsSystem(record, kind, layout, accessors));
        ...
        if (!handWritten)
        {
            ...
            switch (kind)
            {
                ...
                case TypeKind.MiniObject:
                    WriteMiniObject(writer, record, typeName, accessors);
                    break;

                case TypeKind.Boxed:
                    WriteBoxed(writer, module, record, typeName);
                    break;

                default:
                    WriteOpaque(writer, record, typeName);
                    break;
            }
        }
        ...
        bool needsSystem)
    {
        ...
        bool needsCompilerServices = false;
        ...
        // function of a boxed type.
        bool needsInteropServices = layout is not null || kind == TypeKind.Boxed;
    ...
    private static void WriteInlineArray(
        CodeWriter writer,
        string visibility,
        string summary,
        ...
        IReadOnlyList<Accessor> accessors)
    {
        ...
        XmlDocWriter.WriteObsolete(writer, record);
        writer.WriteLine(
            "public sealed " + (accessors.Count > 0 ? "unsafe " : string.Empty) + "partial class " + typeName
            + " : Gst.MiniObject");
        writer.OpenBlock();
    ...
    private void WriteBoxed(CodeWriter writer, ModuleInfo module, GirRecord record, string typeName)
    {
        string getType = typeName + "GetType";
        XmlDocWriter.Write(writer, record.Doc, FallbackSummary(record, "boxed type"));
        XmlDocWriter.WriteObsolete(writer, record);
        writer.WriteLine("public sealed partial class " + typeName + " : Gst.GObject.Boxed");
        writer.OpenBlock();
        ...
        WriteWrapperConstructor(
            ...
            record,
            typeName,
            "base(handle, new Gst.GObject.GType(" + getType + "()), transfer)");
        ...
        writer.WriteLine();
        writer.WriteLine(
            "/// <summary>Returns the <c>GType</c> that GObject registered <c>" + CTypeOf(record)
            + "</c> under.</summary>");
        writer.WriteLine("/// <returns>The boxed type.</returns>");
        writer.WriteLine("[LibraryImport(\"" + module.NativeLibrary + "\", EntryPoint = \"" + record.GlibGetType + "\")]");
        writer.WriteLine("private static partial nuint " + getType + "();");
        writer.CloseBlock();
    }

    private void WriteOpaque(CodeWriter writer, GirRecord record, string typeName)
    {
    ...
    /// <summary>The inline storage of a fixed size array field.</summary>
    /// <param name="TypeName">The name of the generated storage type.</param>
    /// <param name="ElementTypeName">The C# type of one element.</param>
    /// <param name="Length">The number of elements.</param>
    private sealed record InlineArrayInfo(string TypeName, string ElementTypeName, int Length);
