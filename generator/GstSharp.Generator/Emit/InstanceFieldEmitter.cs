...
/// an error, a field that occupies a part of a word rather than the whole of it
/// is an error, and so is a class that grew a nested <c>&lt;union&gt;</c>: the
/// gir keeps that out of the field list, so laying out the fields alone would
/// mirror a structure shorter than the instance is.
/// </para>
/// </remarks>
internal sealed class InstanceFieldEmitter
{
        ...
        IReadOnlyList<InstanceFieldPlan> plans)
    {
        ...
        {
            // A nested union is read into a list of its own, so the loop over
            // the fields below never sees it and the mirror would be short by
            // the whole block. The size probe would then compare a structure the
            // instance is not, which is exactly what it exists to catch.
            _diagnostics.Error(
                "GEN0062",
                $"The class '{cName}' declares the nested union '{union.Name}', which the own fields mirror has "
                + "no storage for; a mirror that left it out would be shorter than the instance is.");
        }
    ...
    private string TypeOf(GirNamespace ns, GirField field)
    {
        ...
        {
            // A bitfield occupies a part of a word that it shares with its
            // neighbours, and a mirror member occupies the whole of one. The two
            // agree on the size of the structure only by coincidence, so the
